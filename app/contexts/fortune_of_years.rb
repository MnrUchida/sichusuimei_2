  ...
  class FortuneValue
    ...
    class << self
      private def get_ranges(person:, range:)
        ...
        return [nil, nil] if date_of_birth > range_max

        [[range_min, date_of_birth].max..range_max, (range_max.year - date_of_birth.year).years]
      end

      private def get_fortune_gogyo(person:, range:)
        fortune_of_decades = person.fortune_of_decades(range: range)
        fortune_of_years = person.fortune_of_years(range: range)
        gogyo_values = fortune_of_decades.gogyo_values + fortune_of_years.gogyo_values
        [fortune_of_years, gogyo_values]
      end
    end
  end
